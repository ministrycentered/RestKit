...
- (PCOManagedObjectLoader*)objectLoaderForObject:(id<NSObject>)object method:(RKRequestMethod)method delegate:(id<PCOManagedObjectLoaderDelegate>)delegate {
    ...
    if (self.inferMappingsFromObjectTypes) {
        PCOManagedObjectMapping* objectMapping = [self.mappingProvider objectMappingForClass:[object class]];
...
- (PCOManagedObjectLoader*)getObject:(id<NSObject>)object delegate:(id<PCOManagedObjectLoaderDelegate>)delegate {
	PCOManagedObjectLoader* loader = [self objectLoaderForObject:object method:RKRequestMethodGET delegate:delegate];
	[loader send];
	return loader;
}

- (PCOManagedObjectLoader*)postObject:(id<NSObject>)object delegate:(id<PCOManagedObjectLoaderDelegate>)delegate {
	PCOManagedObjectLoader* loader = [self objectLoaderForObject:object method:RKRequestMethodPOST delegate:delegate];
	[loader send];
	return loader;
}

- (PCOManagedObjectLoader*)putObject:(id<NSObject>)object delegate:(id<PCOManagedObjectLoaderDelegate>)delegate {
	PCOManagedObjectLoader* loader = [self objectLoaderForObject:object method:RKRequestMethodPUT delegate:delegate];
	[loader send];
	return loader;
}

- (PCOManagedObjectLoader*)deleteObject:(id<NSObject>)object delegate:(id<PCOManagedObjectLoaderDelegate>)delegate {
	PCOManagedObjectLoader* loader = [self objectLoaderForObject:object method:RKRequestMethodDELETE delegate:delegate];
	[loader send];
	return loader;
}
...
- (PCOManagedObjectLoader*)sendObject:(id<NSObject>)object delegate:(id<PCOManagedObjectLoaderDelegate>)delegate block:(void(^)(PCOManagedObjectLoader*))block {
    PCOManagedObjectLoader* loader = [self objectLoaderWithResourcePath:nil delegate:delegate];
    loader.sourceObject = object;
    loader.targetObject = object;
    loader.serializationMIMEType = self.serializationMIMEType;
    loader.serializationMapping = [self.mappingProvider serializationMappingForClass:[object class]];
...
- (PCOManagedObjectLoader*)sendObject:(id<NSObject>)object method:(RKRequestMethod)method delegate:(id<PCOManagedObjectLoaderDelegate>)delegate block:(void(^)(PCOManagedObjectLoader*))block {
    return [self sendObject:object delegate:delegate block:^(PCOManagedObjectLoader* loader) {
        loader.method = method;
        block(loader);
    }];
}
